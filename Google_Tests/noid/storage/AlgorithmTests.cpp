...
#include "storage/Algorithm.h"
#include "storage/BPlusTreeKey.h"
...
TEST_F(AlgorithmFixture, NextLargest) {
  ...
  auto index = noid::storage::NextLargest(vec, 0, static_cast<int64_t>(vec.size() - 1), needle, GetKeyReference);
  EXPECT_EQ(index, 0) << "Expect next-largest element to reside at index 0 for smaller needle";

  needle[BTREE_KEY_SIZE - 1] = 3;
  index = noid::storage::NextLargest(vec, 0, static_cast<int64_t>(vec.size() - 1), needle, GetKeyReference);
