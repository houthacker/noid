...
class NoidFile {
 protected:
     ...
     */
    [[nodiscard]] virtual std::size_t Read(std::vector<byte>& container, std::vector<byte>::iterator container_pos,
 ...
 public:
     ...
     */
    std::size_t WriteContainer(const std::vector<byte>& source, Position start_position) {
     ...
     * @throws std::ios_base::failure if writing @p size bytes to the file fails.
     */
    std::size_t WriteContainer(const std::vector<byte>& source, Position source_start, Position write_start, std::size_t size) {
     ...
     */
    [[nodiscard]]
    std::size_t ReadContainer(std::vector<byte>& destination, Position file_pos, std::size_t size) {
     ...
     */
    [[nodiscard]]
    std::size_t ReadContainer(std::vector<byte>& destination, Position destination_pos, Position file_pos, std::size_t size) {
      auto required_size = destination_pos + size;
      if (required_size > destination.capacity()) {
        destination.reserve(required_size);
      }
