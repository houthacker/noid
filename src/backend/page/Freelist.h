...
class Freelist {
 private:
    ...
    explicit Freelist(std::vector<byte> data);
...
class FreelistBuilder {
 private:
     ...
     */
    std::size_t max_page_position;

    /**
     * @brief The position at which a new free page number is to be stored.
     */
    uint16_t next_free_page_position;
 ...
 public:
     ...
     * @details Adds the given @p free_page to the list of reusable page numbers. If @p position_hint is less than
     ...
     *
     * @param free_page The page number to designate as reusable.
     * @param position_hint A hint as to where to store the given @p free_page.
     * @return A reference to this builder to support a fluent interface.
     * @throws std::overflow_error if the @c Freelist is full before adding the given @p free_page
     */
    FreelistBuilder& WithFreePage(PageNumber free_page, std::size_t position_hint);
};
